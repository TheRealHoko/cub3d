...
/*   Updated: 2021/02/24 19:09:32 by jzeybel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*ft_parse(char *s, t_parse *parse)
{
	...
	while (ret == 1)
	{
		...
		{
			parse->res[0] = ft_atoll(ft_split(f, delim)[1]);
			parse->res[1] = ft_atoll(ft_split(f, delim)[2]);
		}
		else if (ft_strnstr(f, "NO", ft_strlen(f)))
			parse->NO = ft_split(f, delim)[1];
		else if (ft_strnstr(f, "SO", ft_strlen(f)))
			parse->SO = ft_split(f, delim)[1];
		else if (ft_strnstr(f, "WE", ft_strlen(f)))
			parse->WE = ft_split(f, delim)[1];
		else if (ft_strnstr(f, "EA", ft_strlen(f)))
			parse->EA = ft_split(f, delim)[1];
		else if (ft_strnstr(f, "S", ft_strlen(f)))
			parse->S = ft_split(f, delim)[1];
		else if (ft_strnstr(f, "F", ft_strlen(f)))
		{
			parse->F[0] = ft_atoll(ft_split(f, delim)[1]);
			parse->F[1] = ft_atoll(ft_split(f, delim)[2]);
			parse->F[2] = ft_atoll(ft_split(f, delim)[3]);
		}
		else if (ft_strnstr(f, "C", ft_strlen(f)))
		{
			parse->C[0] = ft_atoll(ft_split(f, delim)[1]);
			parse->C[1] = ft_atoll(ft_split(f, delim)[2]);
			parse->C[2] = ft_atoll(ft_split(f, delim)[3]);
		}
		ret = get_next_line(fd, &f);
	}
	close(fd);
